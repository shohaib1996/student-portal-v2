...
export function BeautifulMentionsPlugin(props: BeautifulMentionsPluginProps) {
    ...
    const options = useMemo((): MentionOption[] => {
        ...
        const readyToAddCurrentMentions =
            !onSearch || (!loading && query !== null);
            ...
            editor.getEditorState().read(() => {
                ...
                for (const mention of mentions) {
                    ...
                    if (
                        ...
                        (query === null ||
                            mention.getValue().startsWith(query)) &&
                        opt.every((o) => o.value !== value)
                    ) {
                        opt.push(
                            new MentionOption(trigger, value, value, data),
                        );
                    }
                }
            });
    ...
    const handleSelectOption = useCallback(
        (
            ...
            editor.update(() => {
                ...
                const newMention =
                    !!creatable &&
                    selectedOption.value !== selectedOption.displayValue;
                const value =
                    newMention &&
                    mentionEnclosure &&
                    /\s/.test(selectedOption.value)
                ...
                const trigger =
                    index === -1
                        ...
                        : replaceableString.substring(0, index) +
                          replaceableString.substring(
                              index + matchingString.length,
                          );
