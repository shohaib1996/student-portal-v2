...
type Status = 'completed' | 'upcoming' | 'current';
...
const ContentDropDown = ({
    ...
    const toggleModule = (moduleId: string) => {
        ...
        // Trigger parentId update for fetching children if it's a chapter
        const item = fetchedData?.find((d) => d._id === moduleId);
    ...
    const renderContent = (
        ...
        return data.map((item, index) => {
            ...
            // Handle chapter type content (equivalent to a module)
            if (item.type === ChapterType.CHAPTER && 'chapter' in item) {
                const progress = item.isCompleted ? 100 : 0; // Adjust based on your data
                ...
                return (
                    <div
                        key={uniqueKey}
                        className='border border-border-primary-light rounded-lg overflow-hidden mb-4'
                    >
                        <div
                            className='flex items-center justify-between p-2 cursor-pointer hover:bg-primary-light transition-colors'
                            onClick={() => toggleModule(item._id)}
                        >
                            <div className='flex items-center gap-3'>
                                {expandedModules[item._id] ? (
                                    ...
                                    <div className='flex items-center gap-4 text-sm text-gray-500'>
                                        <span className='flex items-center gap-1'>
                                            <svg
                                                ...
                                                fill='none'
                                            >
                                                <g clipPath='url(#clip0_336_47041)'>
                                                    <path
                                    ...
                                    item.chapter.children.map(
                                        (child, childIndex) => {
                                            if (
                                                ...
                                                const status = child.isCompleted
                                                    ? 'completed'
                                                    : 'upcoming';
                                                return (
                                                    <div
                                                                    ...
                                                                    <Link
                                                                        href={
                                                                            child
                                                                                .lesson
                                                                                .url
                                                                                ? `/dashboard/slide/${child.lesson.url}`
                                                                                : '#'
                                                                        }
                                                                        target='_blank'
                                                                    >
            ...
            // Handle lesson type content directly (no nesting)
            if (item.type === ChapterType.LESSON && 'lesson' in item) {
                ...
                return (
                    <div
                        key={uniqueKey}
                        className={cn(
                            'flex items-center justify-between p-3 border border-border-primary-light rounded-lg mb-4',
